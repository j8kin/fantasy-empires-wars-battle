...
export function BattleLayout() {
  ...
  const zoneFrames = [
    { y: y0,                    h: worldScreenH * attacker,                 zIndex: 150 },
    { y: y1 - THIN_CORNER,      h: worldScreenH * neutral  + THIN_CORNER,   zIndex: 151 },
    { y: y2 - THIN_CORNER,      h: worldScreenH * (1 - attacker - neutral) + THIN_CORNER, zIndex: 152 },
  ];
  ...
  return (
    <>
      ...
      <FantasyBorderFrame
        ...
        frameSize={{ width: ARMY_PANEL_WIDTH, height: h }}
        accessible={true}
        zIndex={100}
      >
        <div style={{ color: '#c8a87a', padding: '12px 8px', fontSize: 14 }}>
          Army Panel
          <br />
          <span style={{ opacity: 0.5, fontSize: 12 }}>(implemented in Step 2)</span>
        </div>
      </FantasyBorderFrame>

      {/* ── Right: battlefield ────────────────────────────────────────────── */}
      {/* Shifted left by CORNER_SIZE so its left border overlaps (replaces)   */}
      {/* the right border of the left panel — only one border visible.        */}
      <FantasyBorderFrame
        screenPosition={{ x: ARMY_PANEL_WIDTH - CORNER_SIZE, y: 0 }}
        frameSize={{ width: w - ARMY_PANEL_WIDTH + CORNER_SIZE, height: h }}
        accessible={true}
        zIndex={101}
      >
