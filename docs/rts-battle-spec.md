...
- Unit count in pack
- Key stats: Health, Attack, Defense, Speed
...
- A dialog appears listing the eligible crew unit types (from `warMachine.eligibleCrewTypes`)
- The player selects one pack of eligible unit type from the army panel to crew it
- Only unit types listed in `warMachine.eligibleCrewTypes` may crew a war machine — this list is always a subset of
  melee types (TBS never puts ranged units there)
- **The entire selected pack loads into the machine** — crew count = that pack's current unit count (up to 20)
- All units of that pack are consumed from the army panel; the pack is no longer available for field deployment
- The machine's combat stats are **derived entirely from the crew's `CombatStats`**:
    ...
    - `baseMoveSpeed = crew.combatStats.speed × machineMoveMultiplier`
    ...
    - Better crew (Veteran/Elite) and larger pack (more units loaded) produce stronger machines
- If no pack of an eligible crew type exists in the army, placement is blocked with a clear error

> **During battle:** a player may also direct a unit pack of an eligible crew type to move to an unoccupied
...
```
damage = max(1, attacker.attack + morale_bonus − defender.defense)
```
...
```
damage = max(1, baseDamage × (1 − distance / maxReachRadius) − defender.defense)
```

**Ranged:**

- Ranged damage uses `combatStats.rangeDamage` as the base; falls back to `combatStats.attack` if absent
...
│  Can be CAPTURED by a pack of eligible crew type.       │
...
`maxCrewHP = crewCount × crewUnit.combatStats.health`
where `crewCount` = number of units in the loaded pack (up to 20).
...
2. An enemy **pack** whose unit type is in `warMachine.eligibleCrewTypes` reaches it → machine is captured
3. The capturing pack becomes the new crew; machine reactivates at `recapturedEffectiveness`
   (configurable — default 70%, representing a fresh but less-drilled crew)
4. **If a unit of an ineligible type reaches it:** machine is **destroyed** — they cannot operate it
...
export const BATTLE_CONFIG = {
    ...
    //   baseMoveSpeed = crew.speed                    × moveMultiplier
    //   baseFireRate  = crew.attackSpeed              × fireRateMultiplier
    // crewCount = number of units in the loaded pack (up to 20).
...
```json
{
  "cleric": {
    "hpRegenPerSec": 3,
    "undeadSurvivalSeconds": 7,
    "undeadTickInterval": 0.5
  }
}
```

`undeadSurvivalSeconds` - how long an Undead survives at full HP inside aura
`undeadTickInterval` - damage tick interval in seconds
...
| OQ-52 | Should `BattleArmy` carry a faction identifier from TBS?                                                                                                | **No.** The battle module is independent of faction identity. DRIVEN-specific mechanics (Command Radius, aura immunity, morale immunity, instant defeat condition) are hardcoded in the RTS per unit type (Warsmith, Golem, Gargoyle, Dendrite). Units fight based on their `CombatStats` values; no faction flag is needed or passed. See §7.10.                                                                                     |
| OQ-53 | Should `RegularsState` carry unit ability identifiers from TBS?                                                                                         | **No.** Unit abilities (Orc Bloodlust, Gargoyle Flight, Golem Wall-Buster, Dendrite Reach Attack) are intrinsic to unit types, not fraction-level rules. They are hardcoded in the RTS per unit type name and are not provided by TBS. See §6.11.                                                                                                                                                                                     |
...
export interface CombatStats {
    ...
    defense: number;       // damage reduction (replaces RTS-only "armor")
    speed: number;         // movement speed (replaces RTS-only "moveSpeed")
    attackSpeed: number;   // attacks per second — required by RTS simulation; TBS sets this per unit type
...
// ── TBS alignment note ────────────────────────────────────────────────────────
// `attackSpeed` and `accuracy` are fields added to the shared CombatStats to support
// RTS battle simulation. TBS must populate these when constructing unit stats during
// faction setup and level-up. `rangeDamage` is provided by TBS for ranged unit types.
...
export interface WarMachineState {
    ...
    durability: number;
    /**
     * Unit types that are eligible to crew (operate) this machine.
     ...
     * Used by the RTS instead of hardcoding crew type knowledge per machine.
     */
...
| War machine       | Eligible crew unit types (`eligibleCrewTypes`)                             | Combat role                                   |
|-------------------|----------------------------------------------------------------------------|-----------------------------------------------|
| **Ballista**      | Warrior / Dwarf (melee — steady precision operators)                       | Long-range piercing, anti-hero                |
| **Catapult**      | Warrior / Dwarf (strong)                                                   | Area siege, wall damage                       |
| **Battering Ram** | Warrior / Orc (heavy)                                                      | Gate/wall only; no ranged attack              |
| **Siege Tower**   | Ward-hands only (melee — agile climbers; Halflings are ranged, ineligible) | Allows units to scale walls; no direct attack |

> **Crew = one full pack.** The entire selected pack loads into the machine (crewCount = pack.count, max 20).
> A larger or higher-rank pack produces a stronger machine. There is no minimum crew count —
> even a partial pack (e.g. 3 surviving Dwarfs) can crew the machine at reduced effectiveness.

> **All war machines require melee unit types as crew.** The eligible unit types are provided
> by TBS via `WarMachineState.eligibleCrewTypes`. The table above is the standard default mapping;
> actual requirements are defined by TBS Faction Restrictions and always contain only melee types.
>
